...
export default function ScheduleCalendar({ appointments, customers, predictions, performanceRecords, onAddAppointment, onOpenAddAppointmentModal, onEditAppointment, onDeleteAppointment, onDeleteMultipleAppointments, onRequestAppointmentAction, todos, onAddTodo, onToggleTodo, onDeleteTodo, onUpdateTodo, dailyReviews, onSaveDailyReview, onDeleteDailyReview, onDeleteMultipleDailyReviews, onOpenAIAddAppointmentModal, onRescheduleFromPostponed, onSelectCustomer, onEditConsultation, onDeleteConsultation, onDeleteMultipleConsultations, onAddPrediction, onUpdatePrediction, isMobile, onOpenConsultationRecordModal }: ScheduleCalendarProps) {
  ...
  return (
    <div>
        ...
        {viewMode === 'calendar' && (
            <div ref={containerRef} className={(isMobile || layoutMode === 'vertical') ? '' : 'xl:flex xl:h-[calc(100vh-240px)]'}>
                <div 
                    className="flex-shrink-0" 
                    style={(isMobile || layoutMode === 'vertical') ? {width: '100%'} : { width: `${leftPaneWidth}%`}}
                >
                        ...
                        <div className="bg-[var(--background-secondary)] p-4 rounded-lg shadow-lg border border-[var(--border-color)] mb-2">
                            <div className="flex items-center justify-between flex-wrap gap-2">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    {!isMobile && (
                                        <div className="flex items-center p-1 bg-[var(--background-tertiary)] rounded-lg ml-2">
                                            <button
                                                onClick={() => setLayoutMode('horizontal')}
                                                className={`p-1.5 rounded-md ${layoutMode === 'horizontal' ? 'bg-[var(--background-secondary)] text-[var(--text-accent)] shadow' : 'text-[var(--text-secondary)]'}`}
                                                title="좌우 분할"
                                            >
                                            ...
                                            <button
                                                ...
                                                className={`p-1.5 rounded-md ${layoutMode === 'vertical' ? 'bg-[var(--background-secondary)] text-[var(--text-accent)] shadow' : 'text-[var(--text-secondary)]'}`}
                                                title="상하 배치"
                                            >
                                                <ViewRowsIcon className="h-4 w-4" />
                                            </button>
                                        </div>
                                    )}
                                </div>
